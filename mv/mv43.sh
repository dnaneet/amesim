#!/bin/bash
#This is a small improvement over mv42.sh.  This mv43.sh checks for any motion file that has been used to apply a motion.
#For Fall 2017 exam 2, it is recommended to use mv42.sh instead of mv43.sh
#
...
for FILENAME in *.mdl
  do
    ...
    if grep -q *SetCurve "$FILENAME"; then
		curveFile=$(grep "*SetCurve" $FILENAME  | awk '{print $4}')
    ...
    printf '%s ' "Number of Translational Joints:" $nTrans "," "Number of Revolute Joints: " $nRev "," "Number of Fixed Joints: " $nFixed "," "Number of Ball Joints:" $nBall "," "Number of Motions:" $nMotion "," "Motion applied to:" $motionOnJoint "Motion type: " $motionType  ","   "Expression:"$motionExp ", " "curve file (if any):" $curveFile >> mdl_file_summary.txt   
    echo $'\n' >> mdl_file_summary.txt
